...
void main() {
  ...
  Map<String, dynamic> passes(String value) => {
        'latin': {'text': value}
      };
  CardCropTextCheck check(String original, String crop) =>
      CardCropTextCheck.evaluate(
          originals: passes(original),
          cropped: passes(crop),
  ...
  test('accepts whitespace and capitalization differences', () {
    expect(
        check(text,
                text.toUpperCase().replaceAll(' ', '  ').replaceAll('\n', ' '))
            .verified,
        isTrue);
  });
  test('rejects a missing contact number even when company text remains', () {
    final result =
        check(text, 'Example Trading Company\nAlex Example\nalex@example.com');
  ...
  test('requires every selected script on both images', () {
    final result = CardCropTextCheck.evaluate(
        originals: passes(text),
        cropped: passes(text),
  ...
  test('rejects failed crop recognition', () {
    final result = CardCropTextCheck.evaluate(
        originals: passes(text), cropped: {}, requiredScripts: {'latin'});
    expect(result.verified, isFalse);
  });
