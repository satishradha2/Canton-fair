...
class SupplierProfile {
  static const companyFields = <String, String>{
    ...
    'localName': 'Company name in local language',
    'brandNames': 'Brands / trading names',
    'supplierType': 'Manufacturer / trader / distributor',
    'category': 'Product category',
    'productsServices': 'Products and services',
    'yearEstablished': 'Year established',
    'websites': 'Websites (one per line)',
    'companyEmails': 'Company emails (one per line)',
    'companyPhones': 'Office phones (one per line)',
    'companyFax': 'Company fax',
    'address': 'Full business addresses',
    ...
    'exportLicense': 'Export licence reference',
    'certifications': 'Certifications shown / reported',
    'exportMarkets': 'Export markets',
    'booth': 'Booth',
    'hall': 'Hall',
    'social': 'Company social profiles',
    'notes': 'Supplier notes',
    'other': 'Additional details / custom field notes',
  };
  static const contactFields = <String, String>{
    'person': 'Contact name',
    'localPerson': 'Contact name in local language',
    'role': 'Designation / job title',
    'department': 'Department',
    'email': 'Primary email',
    'otherEmails': 'Other emails (one per line)',
    'phone': 'Primary phone',
    'otherPhones': 'Other phones (one per line)',
    'whatsapp': 'WhatsApp',
    'wechat': 'WeChat',
    'fax': 'Contact fax',
    'directLine': 'Direct line / extension',
    'contactSocial': 'Contact social profiles',
  ...
  static Map<String, String> contactSubset(Map<String, String> fields) => {
        for (final key in contactFields.keys)
          if (fields.containsKey(key)) key: fields[key]!,
      };
  ...
  static Future<Exhibitor> save({
    ...
    return TeamWorkspaceService.exclusive(() async {
      ...
      final saved = await database.transaction<Exhibitor>((txn) async {
        ...
        capture['supplier_details'] = {
          ...Map<String, dynamic>.from(
              capture['supplier_details'] as Map? ?? {}),
          ...companySubset(normalizedValues),
        };
        if (card != null) {
          final archive = card.archive();
          final history = cards(Exhibitor.fromMap(row))
          ...
          for (final side in card.sides.keys) {
            final files = {
              ...
              if (card.readingPath(side) != card.imagePath(side))
                'Business card $side corrected | ${card.id}':
            ...
            for (final entry in files.entries) {
              ...
              if (linked.any((item) =>
                  ...
                  item['owner_id'] != original.id)) {
                throw StateError(
                    'This card is already attached to another supplier.');
              }
              if (linked.isEmpty) {
                await txn.insert(
                    'attachments',
        ...
        if (saveContact) {
          final profile = originalContact == null
              ? <String, dynamic>{}
              : object(originalContact.profileJson);
          if (originalContact != null) {
            final contacts = await txn.query('contacts',
                where: 'id = ? AND exhibitor_id = ?',
                whereArgs: [originalContact.id, original.id]);
            if (contacts.isEmpty ||
                originalContact.toMap().entries.any(
                    (entry) => contacts.single[entry.key] != entry.value)) {
              throw StateError(
                  'The contact changed elsewhere. Reopen and review before saving.');
            }
          }
          profile['details'] = {
            ...Map<String, dynamic>.from(profile['details'] as Map? ?? {}),
            ...contactSubset(normalizedValues)
          };
          profile['language'] = normalizedValues['language'] ?? '';
          final contact = Contact(
            exhibitorId: original.id!,
            name: (normalizedValues['person'] ?? '').trim().isEmpty
                ? 'Unnamed company contact'
                : normalizedValues['person']!,
            designation: normalizedValues['role'] ?? '',
            phone: normalizedValues['phone'] ?? '',
            email: normalizedValues['email'] ?? '',
            whatsapp: normalizedValues['whatsapp'] ?? '',
            wechat: normalizedValues['wechat'] ?? '',
          ...
          if (originalContact == null) {
            ...
            await txn.update('contacts', contact,
                where: 'id = ?', whereArgs: [originalContact.id]);
          }
        }
        final changes = <String, Object?>{
          for (final key in [
            ...
            key: normalizedValues[key] ?? '',
          'field_capture_json': jsonEncode(capture),
        };
        await txn.update('exhibitors', changes,
      ...
      if (card != null) {
        try {
          await card.markSaved();
        } on FileSystemException catch (_) {/* Originals remain safe. */}
      }
      return saved;
    });
  }
