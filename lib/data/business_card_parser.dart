...
class BusinessCardParser {
  static Map<String, String> parse(String text) {
    ...
    const labels = <String, String>{
      ...
      'registered company name': 'legalName',
      'local company name': 'localName',
      'brand': 'brandNames',
      'business type': 'supplierType',
      'products': 'productsServices',
      'services': 'productsServices',
      'established': 'yearEstablished',
      'company email': 'companyEmails',
      'office phone': 'companyPhones',
      ...
      'export license': 'exportLicense',
      'certifications': 'certifications',
      'export markets': 'exportMarkets',
      'direct line': 'directLine',
      'extension': 'directLine',
    ...
    if (!result.containsKey('name')) {
      final candidates = unlabelled
          .where((line) =>
              !line.contains('@') &&
              !RegExp(r'\b(test card|not a real|street|road|avenue|www)\b',
                      caseSensitive: false)
                  .hasMatch(line) &&
              RegExp(r'\b(supplies|trading|industries|industrial|technology|technologies|manufacturing|shipping|logistics|line|lines|limited|ltd|llc|inc|corporation|company|co)\b',
                      caseSensitive: false)
                  .hasMatch(line))
          .toList();
      if (candidates.length == 1) {
        result['name'] = candidates.single;
      } else {
        final legalNames = candidates
            .where((line) => RegExp(
                  r'\b(llc|ltd|limited|inc|corporation|company|co\.?|gmbh|sarl|pte)\b',
                  caseSensitive: false,
                ).hasMatch(line))
            .toList();
        if (legalNames.length == 1) result['name'] = legalNames.single;
      }
    }
    ...
    if (!result.containsKey('person') && company != null) {
      ...
      for (var index = 0; index < lines.length; index++) {
        ...
        if (!unlabelled.contains(line) ||
            line == company ||
            !nameShape.hasMatch(line) ||
            nonPerson.hasMatch(line)) {
          continue;
        }
        bool isCompanyLine(String value) =>
            value == company ||
  ...
  static Map<String, List<String>> candidates(String text) {
    ...
    for (final entry in parse(text).entries) {
    ...
    for (final line in text.split(RegExp(r'[\r\n]+'))) {
      if (line.contains(RegExp(r'[:\uFF1A]'))) {
        for (final entry in parse(line).entries) {
          add(entry.key, entry.value);
        }
      }
      for (final match in emailPattern.allMatches(line)) {
        add('email', match.group(0)!);
      }
      for (final match in websitePattern.allMatches(line)) {
        add('websites', match.group(0)!.replaceAll(RegExp(r'[.,;]+$'), ''));
      }
      if (!RegExp(r'\b(fax|booth|hall|postal|zip)\b|\u4f20\u771f|\u5c55\u4f4d',
              caseSensitive: false)
          .hasMatch(line)) {
      ...
      if (!line.contains(RegExp(r'[:\uFF1A@]')) &&
          RegExp(r'\b(manager|director|engineer|president|founder|executive)\b',
                  caseSensitive: false)
              .hasMatch(line) &&
          line.length < 85) {
        add('role', line);
      }
      if (RegExp(r'\b(uae|u\.a\.e\.?|united arab emirates)\b',
              caseSensitive: false)
          .hasMatch(line)) {
        add('country', 'United Arab Emirates');
      } else if (RegExp(r'\b(china|p\.?r\.?\s*china)\b', caseSensitive: false)
          .hasMatch(line)) {
        add('country', 'China');
      }
      if (!line.contains('@') &&
          line.length >= 12 &&
          line.length <= 180 &&
          RegExp(r'\b(street|road|avenue|building|floor|suite|room|business bay|district|industrial park|city|province|dubai|guangzhou)\b',
                  caseSensitive: false)
              .hasMatch(line)) {
        add('address', line);
      }
