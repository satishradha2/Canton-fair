...
class CloudSyncService {
  ...
  static const _relations = <String, Map<String, String>>{
    ...
    'supplier_comment': {'exhibitor_id': 'supplier'},
    'rfq': {'trip_id': 'trip'},
    'expense': {'trip_id': 'trip', 'exhibitor_id': 'supplier'},
    'due_diligence': {'exhibitor_id': 'supplier'},
    'workflow_item': {
      'trip_id': 'trip',
  ...
  Future<SyncResult> _sync() async {
    ...
    if (role != 'viewer') {
      ...
      for (final entry in _tables.entries) {
        for (final row in await _rows(db, entry.key)) {
          ...
          if (entry.key == 'product') {
            final plan =
                ApprovalPolicy.jsonObject(payload['purchase_readiness_json']);
            if (['Ready to order', 'Approved for order']
                .contains(plan['status'])) {
              deferredProducts.add(row);
              if ((link['version'] as int) > 0) continue;
              payload['purchase_readiness_json'] =
                  jsonEncode({...plan, 'status': 'Needs review'});
            }
          }
          if (await _upload(db, team.id, entry.key, link, payload, scope)) {
            uploaded++;
          }
        }
      }
      ...
      for (final type in _tables.keys.toList().reversed) {
        for (final deletion in await db.query('sync_deletions',
          ...
          if (await _upload(db, team.id, type, link, _deleted, scope)) {
            uploaded++;
          }
        }
      }
      ...
      for (final row in deferredProducts) {
        ...
        if (link == null ||
            await _hasConflict(db, 'product', link['record_id'] as String)) {
          continue;
        }
        final payload = await _toCloud(
            db, 'product', row, team.id, link['record_id'] as String);
    ...
    for (final entry in _tables.entries) {
      for (final record in remote[entry.key]!) {
        ...
        if (link != null &&
            (link['version'] as int) >= (record['version'] as int)) {
          continue;
        }
        if (link != null) {
          final local = await _localPayload(db, entry.key, link, team.id);
          if (_hash(local) != link['content_hash'] &&
              _hash(local) != _hash(record['payload'])) {
  ...
  Future<TeamWorkspace> _requireTeam() async {
    ...
    if (team == null) {
      throw StateError(
          'Choose a cloud team before syncing. Personal records stay private.');
    }
    Object? version;
    try {
  ...
  Future<void> _assertScope(String scope) async {
    if (await _workspace.scopeKey() != scope) {
      throw StateError(
          'Account or workspace changed; synchronization stopped.');
    }
  }
  ...
  Future<bool> _hasConflict(
          DatabaseExecutor db, String type, String id) async =>
  ...
  Future<Map<String, Object?>> _toCloud(DatabaseExecutor db, String type,
    ...
    for (final entry
        in (_relations[type] ?? const <String, String>{}).entries) {
      ...
      if (localId == null) {
        ...
        if (link == null) {
          throw StateError('Missing ${entry.value} link for $type.');
        }
    ...
    if (type == 'attachment') {
      ...
      if (!['supplier', 'contact', 'product', 'expense']
          .contains(ownerRecordType)) {
        throw StateError('Unsupported attachment owner.');
      }
      final owner = await _link(db, ownerRecordType,
          id: payload.remove('owner_id') as int);
      if (owner == null) {
        throw StateError('Attachment owner has not been synchronized.');
      }
  ...
  Future<Map<String, Object?>> _fromCloud(DatabaseExecutor db, String team,
    ...
    for (final entry
      ...
      if (foreignId == null) {
        ...
        final link =
            await _link(db, entry.value, recordId: foreignId as String);
        if (link == null) {
    ...
    if (type == 'attachment') {
      ...
      if (ownerType == null ||
          ownerId == null ||
          storagePath == null ||
          !['supplier', 'contact', 'product', 'expense'].contains(ownerType) ||
      ...
      if (owner == null) {
        throw StateError('Download the attachment owner first.');
      }
      ...
      final file = File(
          '${root.path}/team_files/${await _workspace.scopeKey()}/$fileKey${path.extension(storagePath)}');
  ...
  Future<bool> _upload(
    ...
    final remote = await _client
        .from('team_records')
        .select('payload,version,record_id')
        .eq('team_id', team)
        .eq('record_type', type)
    ...
    if (remote != null && _hash(remote['payload']) == _hash(payload)) {
      ...
      if (payload['_deleted'] == true) {
        await db.delete('sync_deletions',
    ...
    if (type == 'attachment' && payload['_deleted'] != true) {
      final rows = await db
          .query('attachments', where: 'id = ?', whereArgs: [link['local_id']]);
      ...
      try {
        await _client.storage.from(_bucket).uploadBinary(
            payload['storage_path'] as String, bytes,
            fileOptions: const FileOptions(upsert: false));
      } on StorageException catch (error) {
        if (error.statusCode != '409' && error.statusCode != 'Duplicate') {
          rethrow;
        }
        final existing = await _client.storage
            .from(_bucket)
            .download(payload['storage_path'] as String);
        if (sha256.convert(existing).toString() != payload['file_sha256']) {
          rethrow;
        }
      }
    }
    ...
    try {
      ...
      await _saveLink(
          db, type, link['local_id'] as int, recordId, version, _hash(payload));
      if (payload['_deleted'] == true) {
        await db.delete('sync_deletions',
            where: 'record_type = ? AND local_id = ?',
            whereArgs: [type, link['local_id']]);
  ...
  Future<void> _apply(Database db, String team, String type,
  ...
  Future<void> keepLocalConflict(CloudConflict conflict) =>
      _resolve(conflict, keepLocal: true);
  Future<void> useCloudConflict(CloudConflict conflict) =>
      _resolve(conflict, keepLocal: false);
  ...
  Future<void> _resolve(CloudConflict conflict, {required bool keepLocal}) =>
      TeamWorkspaceService.exclusive(() async {
        ...
        final latest = await _client
            ...
            .select('payload,version,record_id')
            .eq('team_id', team.id)
            .eq('record_type', conflict.recordType)
            .eq('record_id', conflict.recordId)
            .single();
        ...
        final local =
            await _localPayload(db, conflict.recordType, link, team.id);
        if (latest['version'] != conflict.remoteVersion) {
          await _conflict(
              db, team.id, conflict.recordType, link, local, latest);
          throw StateError(
              'Cloud changed again. Refresh and review the latest conflict.');
        }
        if (keepLocal) {
          await ApprovalPolicy.requireWriter();
          final selected = {...link, 'version': latest['version']};
          await _upload(
              db, team.id, conflict.recordType, selected, local, scope);
          final updated =
              await _link(db, conflict.recordType, id: conflict.localId);
          if (updated?['content_hash'] != _hash(local)) {
            throw StateError(
                'The cloud changed again. Review the refreshed conflict.');
          }
        ...
        await db.delete('cloud_sync_conflicts',
            where: 'id = ?', whereArgs: [conflict.id]);
      });
