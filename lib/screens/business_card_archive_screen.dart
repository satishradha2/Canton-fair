...
class BusinessCardArchiveScreen extends StatefulWidget {
  const BusinessCardArchiveScreen(
      {super.key, required this.supplierId, required this.archive});

  final int supplierId;
  final Map<String, dynamic> archive;

  @override
  State<BusinessCardArchiveScreen> createState() =>
      _BusinessCardArchiveScreenState();
}
...
class _BusinessCardArchiveScreenState extends State<BusinessCardArchiveScreen> {
  ...
  String get _transcript {
    ...
    return [
      ...
      widget.archive['raw_text'] as String? ?? '',
    ].join('\n');
  }
  ...
  Future<void> _share() async {
    try {
      ...
      for (final item in images) {
        if (item.note.endsWith(' | ${widget.archive['id']}') &&
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text(
                'Could not share the card. Try copying the text instead.')));
      }
    }
  }
              ...
              body: Center(
                ...
                child: Image.file(File(image.path),
                    errorBuilder: (_, error, stack) => const Text(
                        'Image unavailable locally. Check attachment sync.')),
              )),
            )));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        Text(fields['name'] as String? ?? 'Business card',
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          OutlinedButton.icon(
              ...
              label: const Text('Share card and text')),
          TextButton.icon(
              onPressed: () async {
        ...
        if (widget.archive['back_blank'] == true)
          const ListTile(title: Text('Back confirmed blank / single-sided')),
        const Divider(),
        for (final entry in fields.entries)
          if (entry.value.toString().trim().isNotEmpty)
            ListTile(
        ...
        const Divider(),
        Text('Original recognition output',
            style: Theme.of(context).textTheme.titleLarge),
        for (final reading in widget.archive['ai_readings'] as List? ?? [])
          ExpansionTile(
            ...
            children: [
              ...
              for (final detail in reading['extra_details'] as List? ?? [])
                ListTile(
                    ...
                    subtitle: SelectableText(detail['value'] as String)),
            ],
          ),
        const Text(
            'These are source readings, not verified facts. Reviewed fields above may differ.'),
