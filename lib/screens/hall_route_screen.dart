...
class _HallRouteScreenState extends State<HallRouteScreen> {
  ...
  Future<void> _importMap() async {
    ...
    final destination = '${directory.path}/fair-map-${DateTime.now().millisecondsSinceEpoch}${path.extension(picked.path)}';
  ...
  Future<void> _toggleMissed(Exhibitor supplier) async {
    ...
    await _db.update('exhibitors', supplier.id!,
        {'field_capture_json': jsonEncode(data)});
          ...
          builder: (context, tripSnapshot) {
            ...
            return FutureBuilder<List<Exhibitor>>(
              ...
              builder: (context, supplierSnapshot) {
                ...
                for (final supplier in suppliers.where((item) => item.hall.trim().isNotEmpty)) {
                  grouped.putIfAbsent(supplier.hall.trim(), () => []).add(supplier);
                }
                final halls = grouped.keys.toList()..sort((a, b) => a.compareTo(b));
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    const Text('Walk halls in order. Shortlisted and highly rated suppliers appear first inside each hall.'),
                    const SizedBox(height: 14),
                    DropdownButtonFormField<int?>(
                      initialValue: _tripId,
                      decoration: const InputDecoration(labelText: 'Trip'),
                      items: [
                        const DropdownMenuItem<int?>(value: null, child: Text('All trips')),
                    ...
                    OutlinedButton.icon(
                      ...
                      label: Text(_mapPath == null ? 'Import official hall map' : 'Replace hall map'),
                    ),
                    if (_mapPath != null && File(_mapPath!).existsSync()) ...[
                      const SizedBox(height: 12),
                      ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: Image.file(File(_mapPath!), height: 190, fit: BoxFit.cover),
                      ),
                    ],
                    ...
                    if (halls.isEmpty)
                      const EmptyState(
                        ...
                        message: 'Captured suppliers with a hall are grouped here into a practical walking route.',
                      )
                    else
                      ...halls.map((hall) => SectionPanel(
                            title: 'Stop ${halls.indexOf(hall) + 1}: Hall $hall',
                            subtitle: '${grouped[hall]!.length} supplier${grouped[hall]!.length == 1 ? '' : 's'}',
                            child: Column(
                              children: grouped[hall]!.asMap().entries.map((entry) {
                                ...
                                return ListTile(
                                  ...
                                  subtitle: Text('Booth ${supplier.booth.isEmpty ? 'not recorded' : supplier.booth}'),
                                  trailing: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      if (supplier.shortlisted || supplier.rating >= 4)
                                        const Icon(Icons.priority_high, color: AppColors.amber),
                                      IconButton(
                                        tooltip: missed ? 'Restore planned booth' : 'Mark missed booth',
                                        icon: Icon(missed ? Icons.undo : Icons.flag_outlined,
                                            color: missed ? AppColors.teal : AppColors.muted),
                                        onPressed: () => _toggleMissed(supplier),
                                      ),
                                    ],
                                  ),
                                );
