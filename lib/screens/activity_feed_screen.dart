...
class _ActivityFeedScreenState extends State<ActivityFeedScreen> {
  ...
  Widget build(BuildContext context) => EnterprisePage(
        title: 'Team activity',
        subtitle:
            'All teammates and all suppliers. Open a supplier Timeline for that supplier only.',
        actions: [
          IconButton(
            tooltip: 'Refresh activity',
            icon: const Icon(Icons.refresh),
            onPressed: () =>
                setState(() => _activity = _db.getAuditLogs(limit: 100)),
          ),
        ],
        children: [
          SizedBox(
                ...
                if (records.isEmpty) {
                  return Center(
                    child: EmptyState(
                      ...
                      message:
                          'Capture or update a supplier, then sync to share it with your team.',
                    ),
                  );
                }
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        child: ListTile(
                          ...
                          title:
                              Text(record['action'] as String? ?? 'Activity'),
                          subtitle: Text([
