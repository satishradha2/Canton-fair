...
class _SupplierProfileScreenState extends State<SupplierProfileScreen> {
  final _controllers = {
    for (final key in {
      ...SupplierProfile.companyFields.keys,
      ...SupplierProfile.contactFields.keys
    })
  ...
  Future<void> _load() async {
    try {
      ...
      _saveContact = widget.createContact ||
          _contact != null ||
          (widget.card != null &&
              SupplierProfile.contactFields.keys
                  .any((key) => widget.card!.fields[key]?.isNotEmpty ?? false));
      _setFields(SupplierProfile.company(supplier));
      _setFields(SupplierProfile.contact(_contact));
    } catch (error) {
      _error = error is StateError
          ? error.message.toString()
          : 'Could not load supplier details. Close and retry.';
    } finally {
  ...
  Future<void> _chooseContact(int? id) async {
    if (SupplierProfile.contactFields.keys
        .any((key) => _controllers[key]!.text != (_baseline[key] ?? ''))) {
      final change = await showDialog<bool>(
          context: context,
          builder: (context) => AlertDialog(
                title: const Text('Switch the contact being edited?'),
                content: const Text(
                    'Unsaved contact edits will be replaced by the selected contact. Company edits and the original card are kept.'),
                actions: [
                  TextButton(
                  ...
                  FilledButton(
                      ...
                      child: const Text('Switch contact'))
                ],
              ));
    ...
    setState(() {
      ...
      for (final contact in _contacts) {
        if (contact.id == id) _contact = contact;
      }
      _setFields(SupplierProfile.contact(_contact));
    });
  }
  ...
  Future<void> _save() async {
    ...
    if (_saveContact &&
      ...
      setState(() => _error =
          'Enter at least one contact detail, or switch off Save contact.');
      return;
    }
    final edits = values.entries
        .where((entry) =>
            entry.value != (_baseline[entry.key] ?? '') &&
            (_saveContact ||
                SupplierProfile.companyFields.containsKey(entry.key)))
        .toList();
              ...
              content: SizedBox(
                  ...
                  child: SingleChildScrollView(
                    ...
                    children: [
                      ...
                      if (_saveContact)
                        Text(_contact == null
                            ? 'Create a separate contact record.'
                            : 'Update contact: ${_contact!.name}'),
                      if (widget.card != null)
                        const Text(
                            'Attach this card and retain all previous cards.'),
                      if (edits.isEmpty)
                        const Text('No existing field values will change.'),
    ...
    return Padding(
        ...
        child: Column(children: [
          TextField(
              ...
              keyboardType: entry.key.toLowerCase().contains('email')
              ...
              decoration: InputDecoration(
                  ...
                  helperText: entry.key == 'otherEmails' ||
                          entry.key == 'otherPhones' ||
                          entry.key == 'companyEmails' ||
                          entry.key == 'companyPhones'
                      ? 'Enter one value per line.'
                      : null)),
          if (incoming.isNotEmpty && incoming != (_baseline[entry.key] ?? ''))
            CheckboxListTile(
              contentPadding: EdgeInsets.zero,
              controlAffinity: ListTileControlAffinity.leading,
              title: Text('Use scanned value: $incoming'),
              subtitle: (_baseline[entry.key]?.isNotEmpty ?? false)
                  ? Text('Existing: ${_baseline[entry.key]}')
                  : null,
              ...
              onChanged: _busy
                  ...
                  : (selected) => setState(() {
                        ...
                        _controllers[entry.key]!.text = selected == true
                            ? incoming
                            : _baseline[entry.key] ?? '';
                      }),
            ),
        ...
        body: SafeArea(
          ...
          if (_supplier != null) ...[
            ...
            SwitchListTile(
                ...
                subtitle: const Text(
                    'Contacts remain separate people under this supplier.'),
                value: _saveContact,
                onChanged: _busy
                    ? null
                    : (value) => setState(() => _saveContact = value)),
            if (_saveContact) ...[
              DropdownButtonFormField<int>(
                  key: ValueKey(_contact?.id ?? -1),
                  initialValue: _contact?.id ?? -1,
                  isExpanded: true,
                  decoration:
                      const InputDecoration(labelText: 'Contact to update'),
                  items: [
                    const DropdownMenuItem(
                        value: -1, child: Text('Add a new contact')),
                    for (final contact in _contacts)
                      DropdownMenuItem(
                          value: contact.id!,
                  ...
                  onChanged: _busy
                      ? null
                      : (id) => _chooseContact(id == -1 ? null : id)),
              const SizedBox(height: 16),
            ...
            const Text(
                'Saved locally. Workspace synchronization and existing team permissions still apply.'),
          ],
        ])),
      ));
}
...
Future<int?> chooseCardSupplier(BuildContext context,
    {bool allowCreate = true}) async {
  return Navigator.of(context).push<int>(MaterialPageRoute(
      builder: (_) => _CardSupplierPicker(allowCreate: allowCreate)));
}
              ...
              child: Column(children: [
                if (widget.allowCreate)
                  FilledButton.icon(
                      ...
                      label: const Text('Create a new supplier')),
                const SizedBox(height: 16),
                TextField(
                    decoration: const InputDecoration(
                        labelText: 'Find an existing supplier',
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                              ...
                              subtitle: Text(
                                  'Booth ${supplier.booth} | ${supplier.country} | Trip #${supplier.tripId}'),
