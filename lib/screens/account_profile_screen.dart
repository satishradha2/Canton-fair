  ...
  try {
    final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
              ...
              content: const Text(
                  'Offline records and photos will not be deleted. Sync important changes before logging out. Unsynced records remain on this device and require the same account and workspace to access them again. Other devices stay signed in.'),
              actions: [
                TextButton(
    ...
    try {
      ...
      if (messenger.mounted) {
        messenger.showSnackBar(const SnackBar(
          content: Text(
              'Could not finish logging out. Check your connection and retry. Offline records were not deleted.'),
        ));
      }
    }
  } finally {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        Card(
          ...
          _detail('Email status',
              user.emailConfirmedAt == null ? 'Not confirmed' : 'Confirmed'),
          FutureBuilder<TeamWorkspace?>(
              future: _workspace,
        ...
        OutlinedButton.icon(
            onPressed: () => confirmAccountLogout(context),
            icon: const Icon(Icons.logout),
            label: const Text('Log out')),
        const SizedBox(height: 8),
        const Text('Logging out does not delete offline records or photos.',
